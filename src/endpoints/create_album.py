...
'''
  ...
  payload = {
    ...
    'songs': [
      {'title': string, 'release': 'YYYY-MM-DD', 'duration': 'HH:MM:SS', 'genre': 'genre', 'label_id': 'label_id', 'other_artists': ['artist_id1', 'artist_id2', ...]},
      song_id2,
      ...
    ]
  }
...
def album():
  ...
  for field in ['title', 'release', 'label_id', 'songs']:
    ...
    if results is None: # user is not an artist
      ...
      for i in range(len(payload['songs'])):
        if payload['songs'][i] is dict:
          ...
          if 'title' not in song or 'release' not in song or 'duration' not in song or 'genre' not in song or 'label_id' not in song:
            logger.info(f'Song does not have all fields provided')
            continue
          # check this song exists
          cur.execute("SELECT ismn FROM song WHERE title = %s AND release = %s AND duration = %s AND genre = %s AND label_id = %s", (song['title'], song['release'], song['duration'], song['genre'], song['label_id']))
        ...
        # associate with song
        cur.execute("INSERT INTO album_song (album_id, song_ismn) VALUES (%s, %s)", (album_id, song_id))
